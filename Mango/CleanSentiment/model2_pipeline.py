...
CleanSentiment version: Google Trends + Santiment + Fear & Greed
...
if os.path.exists(GT_FILE):
    ...
    try:
        ...
        if chunks:
            ...
            print(f"  Saved: {len(raw_gt)} rows")
        else:
            raise RuntimeError("No Google Trends data fetched")
    except Exception as e:
        print(f"  ⚠️ Google Trends fetch failed: {e}")
        print(f"  Creating synthetic proxy from BTC price volatility...")
        ...
        raw_gt['Date'] = pd.to_datetime(raw_gt['Date']).dt.normalize()
        raw_gt.to_csv(GT_FILE, index=False)
        print(f"  Proxy saved: {len(raw_gt)} rows")

# (Phase 3 — Santiment removed: API free tier requires paid subscription for historical data)

# ═══════════════════════════════════════════════════════════════
# STEP 4 — Verify All 5 Files
# ═══════════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("STEP 4 — Verifying All Data Files")
print("=" * 60)

files = {
    'OHLCV':         'ohlcv_2010_to_now.csv',
    'On-Chain':      'onchain_and_technicals_2010_to_now.csv',
    'Fear & Greed':  'sentiment_2010_to_now.csv',
    'Google Trends': GT_FILE,
}

all_good = True
for name, fname in files.items():
    if os.path.exists(fname):
        df_check = pd.read_csv(fname, parse_dates=['Date'])
        print(f"  ✅ {name}: {len(df_check)} rows | "
              f"{df_check['Date'].min().date()} to {df_check['Date'].max().date()}")
    else:
        print(f"  ❌ MISSING: {fname}")
        all_good = False

assert all_good, "Fix missing files before continuing."
print("\n  ✅ All files verified. Proceeding to model pipeline.\n")
...
# Two sources, two masks
df['trends_available'] = (df['Date'] >= '2013-01-01').astype(float)  # always 1 in our range
df['fg_available']     = (df['Date'] >= '2018-02-01').astype(float)

# Zero-fill Fear & Greed for pre-2018 rows
...
for col in fg_cols:
    if col in df.columns:
        ...
        # Post-2018: replace fake neutral 50.0 gaps with forward-fill
...
print(f"  Rows with Fear & Greed (2018+):  {(df['fg_available'] == 1).sum()}")
...
HEAD2_FEATURES = [
    ...
    'gt_change7',
    'gt_momentum',
    # Fear & Greed (2018+, zero-masked before)
    'fear_greed',
    'fg_ma7',
    'fg_ma14',
    'fg_change',
    'fg_change7',
    'fg_extreme_fear',    # binary — do not scale
    'fg_extreme_greed',   # binary — do not scale
    # Availability flags — always include, never scale
    'fg_available',
    'trends_available',
]
sentiment_feature_cols = [c for c in HEAD2_FEATURES if c in df.columns]
...
# HEAD 2: Sentiment — separate bounded, continuous, and flag columns
# Bounded 0-100 → MinMaxScaler
bounded_sentiment = ['fear_greed', 'fg_ma7', 'fg_ma14',
                     'google_trends', 'gt_ma7', 'gt_ma30']

# Continuous derivatives → StandardScaler
continuous_sent = ['fg_change', 'fg_change7', 'gt_change7', 'gt_momentum']

# Binary/flags → no scaling
flag_cols = ['fg_extreme_fear', 'fg_extreme_greed', 'fg_available', 'trends_available']

bounded_sentiment = [c for c in bounded_sentiment if c in HEAD2_FEATURES]
continuous_sent   = [c for c in continuous_sent if c in HEAD2_FEATURES]
...
print(f"\n  === MODEL 2 (CleanSentiment) TEST RESULTS ===")
...
config = {
    'model': 'DualHeadTransformer',
    'name': 'Model 2 — CleanSentiment (Google Trends + Santiment + F&G)',
    'start_date': '2013-01-01',
    'train_end': TRAIN_END,
    'val_end': VAL_END,
    'seq_len': SEQ_LEN,
...
ax.set_title('Model 2 (CleanSentiment) — Training Curves')
ax.legend()
...
print("\n  🎉 MODEL 2 (CleanSentiment) PIPELINE COMPLETE!")
