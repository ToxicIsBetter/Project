...
if not os.path.exists("../../data/clean_google.csv"):
    raise FileNotFoundError("../../data/clean_google.csv is missing.")
...
onchain = pd.read_csv(
    "../../data/clean_onchain.csv", parse_dates=["Date"]
)
...
val = (
    df[(df["Date"] > TRAIN_END) & (df["Date"] <= VAL_END)].copy().reset_index(drop=True)
)
...
boruta = BorutaPy(
    estimator=rf, n_estimators="auto", random_state=42, verbose=0, max_iter=100
)
...
lasso = LogisticRegression(
    penalty="l1", solver="liblinear", C=0.05, random_state=42, max_iter=2000
)
lasso.fit(X_fs, y_fs)
...
if len(boruta_selected) == 0:
    print(
        "  Boruta found NO robust on-chain signals. Falling back to LASSO variables to force on-chain structure."
    )
...
train_h1 = (
    np.hstack([train_h1_scaled, train[head1_binary_cols].values])
    if head1_binary_cols
    else train_h1_scaled
)
val_h1 = (
    np.hstack([val_h1_scaled, val[head1_binary_cols].values])
    if head1_binary_cols
    else val_h1_scaled
)
test_h1 = (
    np.hstack([test_h1_scaled, test[head1_binary_cols].values])
    if head1_binary_cols
    else test_h1_scaled
)
...
N_ONCHAIN = max(
    X1_train.shape[2], 1
)  # Prevent empty feature dimension crashing nn.Linear if empty
...
train_loader = make_loader(
    X1_train, X2_train, y_train_seq, batch_size=32, shuffle=False
)
