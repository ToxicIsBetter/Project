"""
MODEL 4 — Single-Head Transformer: PURE ON-CHAIN ONLY (2018-2026)
Zero Sentiment, Zero Technicals.
"""
import time
import os
import json
...
os.chdir(WORK_DIR)
os.makedirs('processed_model4', exist_ok=True)
...
signed_log_cols = ['TxCnt_growth7d', 'TxCnt_growth30d', 'HashRate_growth7d', 'HashRate_growth30d', 'AdrActCnt_growth7d', 'AdrActCnt_growth30d', 'CapMVRVCur_growth7d', 'CapMVRVCur_growth30d']
...
print(f"  Single Head candidate features (Pure On-Chain ONLY): {len(pure_onchain_cols)}")

print("\n" + "=" * 60)
print("PHASE 12 — Feature Selection")
print("=" * 60)
scaler_temp   = StandardScaler()
X_fs = scaler_temp.fit_transform(train[pure_onchain_cols].values)
...
lasso_selected = [pure_onchain_cols[i] for i, s in enumerate(lasso_mask) if s]
print(f"  LASSO selected {len(lasso_selected)} pure on-chain features")

HEAD1_FEATURES = lasso_selected

with open('processed_model4/feature_sets.json', 'w') as f:
    json.dump({
...
if HEAD1_FEATURES:
    ...
    train_h1 = np.zeros((len(train), 1))
    val_h1   = np.zeros((len(val), 1))
    test_h1  = np.zeros((len(test), 1))

joblib.dump(scaler_h1,     'processed_model4/scaler_head1.pkl')
...
def build_sequences(h1_data, targets, seq_len):
    X1, y = [], []
...
X1_train, y_train_seq = build_sequences(train_h1, y_train, SEQ_LEN)
X1_val,   y_val_seq   = build_sequences(val_h1,   y_val,   SEQ_LEN)
X1_test,  y_test_seq  = build_sequences(test_h1,  y_test,  SEQ_LEN)

np.save('processed_model4/X1_train.npy', X1_train)
np.save('processed_model4/y_train.npy',  y_train_seq)
np.save('processed_model4/X1_val.npy',   X1_val)
np.save('processed_model4/y_val.npy',    y_val_seq)
np.save('processed_model4/X1_test.npy',  X1_test)
np.save('processed_model4/y_test.npy',   y_test_seq)

print("\n" + "=" * 60)
print("PHASE 15 — Building Single-Head Transformer")
print("=" * 60)
...
class SingleHeadTransformer(nn.Module):
    def __init__(self, n_onchain, d_model=64, nhead=4, num_layers=2, dim_feedforward=128, dropout=0.3):
        super().__init__()
...
def make_loader(X1, y, batch_size=32, shuffle=False):
    dataset = TensorDataset(
        torch.tensor(X1, dtype=torch.float32),
        torch.tensor(y, dtype=torch.float32))
    return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle)

train_loader = make_loader(X1_train, y_train_seq, batch_size=32, shuffle=False)
val_loader   = make_loader(X1_val,   y_val_seq,   batch_size=32, shuffle=False)
...
for epoch in range(1, EPOCHS + 1):
    model.train()
    epoch_loss = 0
    for X1b, yb in train_loader:
        X1b, yb = X1b.to(device), yb.to(device)
        optimizer.zero_grad()
        preds = model(X1b)
...
torch.save(best_model_state, 'processed_model4/model4_best.pt')
...
model.eval()
all_preds, all_probs = [], []
test_loader = make_loader(X1_test, y_test_seq, batch_size=32, shuffle=False)

with torch.no_grad():
    for X1b, yb in test_loader:
        X1b = X1b.to(device)
        probs = model(X1b).cpu().numpy()
...
np.save('processed_model4/model4_probs_test.npy',  np.array(all_probs))
np.save('processed_model4/model4_preds_test.npy',  np.array(all_preds))
np.save('processed_model4/model4_ytrue_test.npy',  y_true)

fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(train_losses, label='Train Loss', color='#2196F3')
ax.plot(val_losses, label='Val Loss', color='#FF5722')
ax.set_title('Model 4 (Single Head On-Chain) — Training Curves')
ax.legend()
fig.savefig('plots/model4_training_curves.png', dpi=150)
plt.close(fig)

print("\n  🎉 MODEL 4 FULL ON-CHAIN PIVOT COMPLETE!")
