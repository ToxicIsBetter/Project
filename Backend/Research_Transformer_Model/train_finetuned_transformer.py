"""
FINE-TUNED Dual-Head Transformer
- Soft Class Weighting (Square Root Scaling)
...
print("="*60)
print("FINE-TUNED Dual-Head Transformer")
print("Soft Weights + Threshold Calibration + Momentum")
print("="*60)
...
# --- MOMENTUM FEATURES (Step C) ---
print("\n2. Adding Momentum Features...")
...
print("  Added: momentum_7d, momentum_14d, momentum_30d, volatility_14d, ma_ratio")
...
# Add momentum features to the pool
momentum_cols = ['momentum_7d', 'momentum_14d', 'momentum_30d', 'volatility_14d', 'ma_ratio']
...
# LASSO selection on On-Chain + Momentum
all_candidates = pure_onchain_cols + momentum_cols
scale_onchain = [c for c in all_candidates if c not in binary_cols]
...
# Model
print("\n7. Building Dual-Head Transformer")
...
class DualHeadTransformer(nn.Module):
    def __init__(self, input_dim_h1, input_dim_h2, d_model=64, nhead=8, num_layers=2, dropout=0.3): # Enhanced dropout
        super().__init__()
...
# Training with SOFT Class Weighting (Step A)
print("\n8. Training with Soft Class Weighting")
n_up = y_train.sum()
n_down = len(y_train) - n_up
# Square root scaling for softer weighting
weight_up = np.sqrt(len(y_train) / (2 * n_up))
weight_down = np.sqrt(len(y_train) / (2 * n_down))
...
# Threshold Calibration (Step B)
print("\n9. Threshold Calibration on Validation Set")
model.eval()
with torch.no_grad():
    val_out = model(X1_val_t, X2_val_t).squeeze()
...
with open('processed_model3/metrics_finetuned.json', 'w') as f:
...
plt.legend()
plt.savefig('plots/training_curves_finetuned.png', dpi=150)
plt.close()
...
plt.title('ROC Curve')
plt.legend()
plt.savefig('plots/roc_curve_finetuned.png', dpi=150)
plt.close()
...
print(f"Metrics: processed_model3/metrics_finetuned.json")
