...
print("="*60)
print("MODEL3_OnChain - FIXED TRAINING")
print("With Class Weighting & Threshold Tuning")
print("="*60)
...
trends = pd.read_csv("../../data/train_google.csv", parse_dates=["Date"])
...
print(f"  After filtering (>{START_DATE}): {len(df)} rows")
...
print(f"  Final clean: {df.shape}")

# Check class balance
class_counts = df['target'].value_counts()
print(f"\n  Class distribution:")
print(f"    Up (1):   {class_counts.get(1, 0):,} ({100*class_counts.get(1, 0)/len(df):.1f}%)")
print(f"    Down (0): {class_counts.get(0, 0):,} ({100*class_counts.get(0, 0)/len(df):.1f}%)")
...
# LASSO selection (Boruta found 0 last time)
...
print(f"\n  ✅ Head 1: {len(HEAD1_FEATURES)} features")
print(f"  ✅ Head 2: {len(HEAD2_FEATURES)} features")
...
# Chronological split
TRAIN_END_IDX = int(len(X1_all) * 0.7)
...
print(f"  Train class dist: Up={y_train.sum()}/{len(y_train)} ({100*y_train.sum()/len(y_train):.1f}%)")
...
model_h1_dim = len(HEAD1_FEATURES)
model_h2_dim = len(HEAD2_FEATURES)
model = DualHeadTransformer(model_h1_dim, model_h2_dim)

print(f"  Head 1: {model_h1_dim} | Head 2: {model_h2_dim}")
...
# Weighted BCE loss
class_weights = torch.FloatTensor([weight_down, weight_up])
...
metrics = {
    ...
    'feature_cols': HEAD1_FEATURES + HEAD2_FEATURES,
}
...
print("="*60)
print(f"\nModel saved: processed_model3/model3_best.pt")
print(f"Metrics saved: processed_model3/metrics.json")
